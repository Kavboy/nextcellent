...
if (! class_exists('nggLoader') ) {
    ...
    class nggLoader
    {
        ...
        function start_plugin()
        {
            ...
            if (get_option('ngg_db_version') != NGG_DBVERSION && isset($_GET['page']) != 'nextcellent' ) {
                ...
                if (! empty($ngg_options['silentUpgrade']) ) {
                    include_once dirname( __FILE__ ) . 'src/admin/functions.php';
                    include_once dirname( __FILE__ ) . 'src/admin/upgrade.php';
                    try {
                        ngg_upgrade();
                    } catch ( Exception $e ) {
                        add_action(
        ...
        function required_version()
        {
            ...
            if (( $wp_ok == false ) ) {
                add_action(
                    ...
                    function () {
                        global $ngg;
                        printf('<div id="message" class="error"><p><strong>' . __('Sorry, NextGEN Gallery works only under WordPress %s or higher', "nggallery") . '</strong></p></div>', $ngg->minimum_WP);
                    }
                );
                return false;
            }
        ...
        function load_dependencies()
        {
            ...
            require_once dirname( __FILE__ ) . '/src/lib/core.php';                  // 94.840
	        require_once dirname( __FILE__ ) . '/src/lib/ngg-db.php';                    // 132.400
	        require_once dirname( __FILE__ ) . '/src/lib/image.php';                 // 59.424
	        require_once dirname( __FILE__ ) . '/src/lib/tags.php';                  // 117.136
	        require_once dirname( __FILE__ ) . '/src/lib/post-thumbnail.php';            // n.a.
	        require_once dirname( __FILE__ ) . '/src/widgets/class-ngg-slideshow-widget.php';
	        require_once dirname( __FILE__ ) . '/src/widgets/class-ngg-media-rss-widget.php';
	        require_once dirname( __FILE__ ) . '/src/widgets/class-ngg-gallery-widget.php';
	        require_once dirname( __FILE__ ) . '/src/lib/multisite.php';
	        require_once dirname( __FILE__ ) . '/src/lib/sitemap.php';

            // Load frontend libraries
	        require_once dirname( __FILE__ ) . '/src/lib/navigation.php';                // 242.016
	        require_once dirname( __FILE__ ) . '/nggfunctions.php';              // n.a.
	        require_once dirname( __FILE__ ) . '/src/lib/shortcodes.php';                // 92.664
            ...
            if (defined('XMLRPC_REQUEST') ) {
	            require_once dirname( __FILE__ ) . '/src/lib/xmlrpc.php';
            }

            // We didn't need all stuff during a AJAX operation
            if (defined('DOING_AJAX') ) {
	            require_once dirname( __FILE__ ) . '/src/admin/ajax.php';
            } else {
	            require_once dirname( __FILE__ ) . '/src/lib/meta.php';              // 131.856
	            require_once dirname( __FILE__ ) . '/src/lib/media-rss.php';         // 82.768
	            require_once dirname( __FILE__ ) . '/src/lib/rewrite.php';               // 71.936
                include_once dirname( __FILE__ ) . '/src/admin/tinymce/tinymce.php';     // 22.408

                // Load backend libraries
                if (is_admin() ) {
	                require_once dirname( __FILE__ ) . '/src/admin/class-ngg-admin-launcher.php';
	                require_once dirname( __FILE__ ) . '/src/admin/media-upload.php';
        ...
        function load_scripts()
        {
            ...
            if (NGGALLERY_IREXIST == true && $this->options['enableIR'] == '1' && nggGallery::detect_mobile_phone() === false ) {
                ...
                wp_enqueue_script('owl', NGGALLERY_URLPATH . 'public/js/owl.carousel.min.js', array( 'jquery' ), '2');
            }
            ...
            if ($nxc ) {
                wp_enqueue_script('qunit-init', NGGALLERY_URLPATH . 'public/js/nxc.main.js', array( 'jquery' )); // main q-unit call
                wp_enqueue_script('qunit', NGGALLERY_URLPATH . 'public/js/qunit-1.16.0.js', array( 'jquery' )); // qunit core
                wp_enqueue_script('nextcellent-test', NGGALLERY_URLPATH . 'public/js/nxc.test.js', array( 'jquery' )); // unit testing specific for nextcellent
            }
        ...
        function multisite_new_blog( $blog_id, $user_id, $domain, $path, $site_id, $meta )
        {
            ...
            include_once dirname( __FILE__ ) . 'src/admin/class-ngg-installer.php';
         ...
         * Activation hook
         * register_activation_hook( $this->plugin_name, array(&$this, 'activate') );
         * Disable Plugin if PHP version is lower than 5.4
         * However, why the plugin spread initial validation over so different places? Not need to do that...
         */
        function activate()
        {
            global $wpdb;
            // Starting from version 1.9.37 it's works only with PHP5.4
            if (version_compare(PHP_VERSION, '5.4.0', '<') ) {
                deactivate_plugins($this->plugin_name); // Deactivate ourself
                wp_die("Sorry, but you can't run this plugin, it requires PHP 5.4 or higher.");
                return;
            }
        ...
        function disable_upgrade( $option )
        {
            ...
            if (isset($option->response[ $this->plugin_name ]) ) {
                ...
                if ($this->add_PHP5_notice == false ) {
                    add_action(
                        "in_plugin_update_message-$this->plugin_name", function () {
                            echo '<br /><span style="color:#ff0000">Please update to PHP5.2 as soon as possible, the plugin is not tested under PHP4 anymore</span>';
                        }
                    );
        ...
        function handle_upload_request()
        {
            if (isset($_GET['nggupload']) ) {
                require_once __DIR__ . '/src/admin/upload.php';
