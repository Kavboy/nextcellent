...
shutterReloaded = {
	...
	init(a) {
		let L, T, ext, i, m, setid, inset, shfile, shMenuPre, k, img;
		...
		for (i = 0; i < document.links.length; i++) {
			...
			if (
				a == 'sh' &&
				L.className.toLowerCase().indexOf('shutter') == -1
			) {
				continue;
			}
	...
	make(ln, fs) {
		...
		const dv = this.textBtns ? ' | ' : '';
		if (shutterLinks[ln].num > 1) {
			prev = shutterSets[shutterLinks[ln].set][shutterLinks[ln].num - 2];
			prevlink =
				'<a href="#" id="prevpic" onclick="shutterReloaded.make(' +
				prev +
				');return false">&lt;&lt;</a>' +
				dv;
			previmg = new Image();
		...
		if (
			...
			nextlink =
				'<a href="#" id="nextpic" onclick="shutterReloaded.make(' +
				next +
				');return false">&gt;&gt;</a>' +
				dv;
			nextimg = new Image();
		...
		D.innerHTML =
			...
			'" id="shTopImg" title="' +
			this.msgClose +
			'" onload="shutterReloaded.showImg();" onclick="shutterReloaded.hideShutter();" />' +
			NavBar +
			'</div>';
	...
	loading() {
		let S, WB, W;
		if ((W = this.I('shWrap')) && W.style.visibility == 'visible') {
			return;
		}
		if (!(S = this.I('shShutter'))) {
			return;
	...
	_viewPort() {
		const wiH = window.innerHeight ? window.innerHeight : 0;
		const dbH = document.body.clientHeight ? document.body.clientHeight : 0;
		const deH = document.documentElement
		...
		if (wiH > 0) {
			...
			this.wHeight =
				this.wHeight - deH > 1 && this.wHeight - deH < 30
					? deH
					: this.wHeight;
		} else {
			this.wHeight = deH > 0 ? deH : dbH;
		}

		const deW = document.documentElement
			? document.documentElement.clientWidth
			: 0;
		const dbW = window.innerWidth
			? window.innerWidth
	...
	handleArrowKeys(e) {
		...
		const nextlink = document.getElementById('prevpic');
		const prevlink = document.getElementById('nextpic');
		const closelink = document.getElementById('shTopImg');
