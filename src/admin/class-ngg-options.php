...
class NGG_Options extends NGG_Post_Admin_Page {
	...
	private function convert_fx_to_optgroup( $data, $title = null, $ngg_options ) {
		...
		foreach ( $data as $option ) {
			$out .= '<option value="' . $option . '" ' . selected(
				$option,
				$ngg_options['slideFx']
			) . '>' . $option . '</option>';
		}
	...
	private function rebuild_slugs() {
		...
		$messages = array(
			'images' => __( 'Rebuild image structure : %1$s / %2$s images', 'nggallery' ),
			'gallery' => __( 'Rebuild gallery structure : %1$s / %2$s galleries', 'nggallery' ),
			'album' => __( 'Rebuild album structure : %1$s / %2$s albums', 'nggallery' ),
		);
