...
function ngg_ajax_dashboard() {
	include_once dirname( dirname( __FILE__ ) ) . '/admin/class-ngg-admin-launcher.php';
	include_once dirname( dirname( __FILE__ ) ) . '/admin/class-ngg-overview.php';
...
function ngg_ajax_image_check() {
	...
	$result = array(
		'stat' => 'ok',
		'message' => sprintf( __( 'Could create image with %1$s x %2$s pixel', 'nggallery' ), $sizes[ $step ]['width'], $sizes[ $step ]['height'] ),
	);
...
function ngg_ajax_image_manager() {
	include_once 'manage/actions.php';
	wp_die();
