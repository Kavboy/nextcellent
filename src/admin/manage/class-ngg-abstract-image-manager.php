			...
			foreach ( $pictures as $pid ) {
				...
				if ( $image ) {
					...
					if ( $image->alttext != $alttext[ $image->pid ] ) {
						...
						$image->image_slug = nggdb::get_unique_slug(
							sanitize_title( $image->alttext ),
							'image',
							$image->pid
						);
					}
					...
					$wpdb->query(
						$wpdb->prepare(
							"UPDATE $wpdb->nggpictures SET image_slug = '%s', alttext = '%s', description = '%s', exclude = %d, imagedate = %s WHERE pid = %d",
							$image->image_slug,
							$image->alttext,
							$image->description,
							$image->exclude,
