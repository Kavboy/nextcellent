...
class NGG_Image_List_Table extends WP_List_Table {
	...
	protected function column_thumbnail( $item ) {
		...
		) . '" class="shutter" title="' . esc_attr( $item->filename ) . '">';
		$out .= '<img class="thumb" src="' . esc_url(
			add_query_arg(
				'i',
				mt_rand(),
				$item->thumbURL
	...
	protected function column_filename( $item ) {
		...
		<?php if ( ! empty( $item->meta_data ) ) { ?>
			<br><?php echo $item->meta_data['width']; ?> x <?php echo $item->meta_data['height']; ?>
						   <?php
							_e(
								'pixel',
								'nggallery'
	...
	protected function column_alt_title_desc( $item ) {
		...
		$out = '<input placeholder="' . __(
			'Alt & title text',
			'nggallery'
		) . '" name="alttext[' . $item->pid . ']" type="text" style="width:95%; margin-bottom: 2px;" value="' . $img_alt_text . '"/>';
		$out .= '<br>';
		$out .= '<textarea placeholder="' . __(
			'Description',
			'nggallery'
		) . '" name="description[' . $item->pid . ']" style="width:95%; margin: 1px;" rows="2">' . $img_description . '</textarea>';
	...
	protected function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'tags':
				...
				return '<textarea placeholder="' . __(
					'Separated by commas',
					'nggallery'
				) . '" name="tags[' . $item->pid . ']" style="width:95%;" rows="2">' . $item->tags . '</textarea>';
			case 'exclude':
	...
	private function get_row_actions( $item ) {
		...
		$actions = array(
			'view'         => '<a class="shutter" href="' . esc_url( $item->imageURL ) . '" title="' . esc_attr(
				sprintf(
					__( 'View "%s"' ),
					sanitize_title( $item->filename )
				)
			) . '">' . __( 'View', 'nggallery' ) . '</a>',
			'meta'         => '<a class="ngg-dialog" data-action="show_meta" data-id="' . $item->pid . '" href="#" title="' . __(
				'Show Meta data',
				'nggallery'
			) . '">' . __( 'Meta', 'nggallery' ) . '</a>',
			'custom_thumb' => '<a class="ngg-dialog" data-action="edit_thumb" data-id="' . $item->pid . '" href="#" title="' . __(
				'Customize thumbnail',
				'nggallery'
			) . '">' . __( 'Edit thumb', 'nggallery' ) . '</a>',
			'rotate'       => '<a class="ngg-dialog" data-action="rotate" data-id="' . $item->pid . '" href="#" title="' . __(
				'Rotate',
				'nggallery'
			) . '">' . __( 'Rotate', 'nggallery' ) . '</a>',
		);
