...
class NGG_Image_Manager extends NGG_Abstract_Image_Manager {
	...
	private function print_gallery_overview( $images ) {
		...
		if ( $options['galSort'] != 'sortorder' ) {
			...
			$disabled = 'disabled ';
			$title    = "title='" . __(
				'To enable manual Sort set Custom Order Sort.See Settings->Gallery Settings->Sort Options',
				'nggallery'
			) . "'";
		}
		?>
					...
					<div class="inside">
						<table class="form-table" id="gallery-properties">
							...
							<tr>
								...
								<td align="right"><label for="preview_pic">
								<?php
								_e(
									'Preview image',
									'nggallery'
							...
							<tr>
								...
								<?php if ( current_user_can( 'publish_pages' ) ) { ?>
									<td align="right"><label for="parent_id">
									<?php
									_e(
										'Create new page',
										'nggallery'
									...
									<td align="left">
										...
										<input class="button-secondary action" type="submit" name="add-new-page" value="
										<?php
										_e(
											'Add page',
											'nggallery'
	...
	private function handle_update_gallery() {
		...
		if ( nggGallery::current_user_can( 'NextGEN Edit gallery options' ) ) {
			...
			if ( nggGallery::current_user_can( 'NextGEN Edit gallery title' ) ) {
				...
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE $wpdb->nggallery SET title= '%s', slug= '%s' WHERE gid = %d",
			...
			if ( nggGallery::current_user_can( 'NextGEN Edit gallery path' ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE $wpdb->nggallery SET path= '%s' WHERE gid = %d",
			...
			if ( nggGallery::current_user_can( 'NextGEN Edit gallery description' ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE $wpdb->nggallery SET galdesc= '%s' WHERE gid = %d",
						esc_attr( $_POST['gallery_desc'] ),
						$this->id
					)
				);
			}
			if ( nggGallery::current_user_can( 'NextGEN Edit gallery page id' ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE $wpdb->nggallery SET pageid= '%d' WHERE gid = %d",
						(int) $_POST['page_id'],
						$this->id
					)
				);
			...
			if ( nggGallery::current_user_can( 'NextGEN Edit gallery preview pic' ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE $wpdb->nggallery SET previewpic= '%d' WHERE gid = %d",
						(int) $_POST['preview_pic'],
						$this->id
					)
				);
			}
			if ( isset( $_POST['author'] ) && nggGallery::current_user_can( 'NextGEN Edit gallery author' ) ) {
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE $wpdb->nggallery SET author= '%d' WHERE gid = %d",
						(int) $_POST['author'],
						$this->id
					)
				);
	...
	private function create_page() {
		...
		if ( $gallery_pageid != 0 ) {
			...
			wp_cache_delete( $this->id, 'ngg_gallery' );
			nggGallery::show_message( sprintf( __( 'New page <strong>%1$s</strong> (ID: %2$s) created.', 'nggallery' ), $gallery_title, $gallery_pageid ) );
		}
