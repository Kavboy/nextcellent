...
include_once( NGGALLERY_ABSPATH . '/src/admin/interface-ngg-displayable.php' );
...
class NGG_Sort_Manager implements NGG_Displayable {
	...
	function display() {
		...
		if ( $options['galSort'] != 'sortorder' ) {
			...
			echo '<a href="' . self::BASE . '&mode=image&gid=' . $this->id . '">' . __(
				'Go back',
				'nggallery'
			) . '</a>';
			...
			<form id="sort_gallery" method="POST" action="<?php echo $clean_url; ?>" accept-charset="utf-8">
				<div class="tablenav">
					<div class="alignleft actions">
						...
						<input class="button-primary action" type="submit" name="update_sort_order" onclick="saveImageOrder()" value="
						<?php
						_e(
							'Update Sort Order',
							'nggallery'
