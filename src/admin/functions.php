...
class nggAdmin {
	 ...
	 * @param bool   $output      (optional) If the function should show an error message or not.
	...
	static function import_gallery( $galleryfolder ) {
		...
		$test_images = self::scandir( $gallerypath );
		...
		self::do_ajax_operation( 'create_thumbnail', array_merge( $image_ids, $updated ), __( 'Create new thumbnails', 'nggallery' ) );
	...
	static function create_thumbnail( $image, $width = 0, $height = 0, $fix = null ) {
		...
		self::maybe_import_meta( $image->pid );
	...
	static function recover_image( $image ) {
		...
		include_once NGGALLERY_ABSPATH . '/src/lib/meta.php';
	...
	static function import_MetaData( $imagesIds ) {
		...
		include_once NGGALLERY_ABSPATH . '/src/lib/image.php';
		...
		foreach ( $imagesIds as $imageID ) {
			...
			if ( ! $image->error ) {
				...
				$meta = self::get_MetaData( $image->pid );
	...
	static function import_zipfile( $galleryID ) {
		...
		$result = self::unzip( $newfolder, $temp_zipfile );
		@unlink( $temp_zipfile );
		...
		if ( $result ) {
			...
			self::import_gallery( $foldername );
			nggGallery::show_message( $message );
		}
	...
	static function upload_images() {
		...
		// read list of images
		$dirlist = self::scandir( $gallery->abspath );
			...
			foreach ( $imagefiles['name'] as $key => $value ) {
				...
				if ( $imagefiles['error'][ $key ] == 0 ) {
					...
					if ( ! @move_uploaded_file( $temp_file, $dest_file ) ) {
						   ...
						   self::check_safemode( $gallery->abspath );
						   continue;
					}
		...
		if ( count( $imageslist ) > 0 ) {
			...
			// add the preview image if needed
			self::set_gallery_preview( $galleryID );
	...
	static function check_upload_image( $galleryID = 0 ) {
		...
		// read list of images
		$imageslist = self::scandir( WINABSPATH . $gallery->path );
		...
		if ( ! @move_uploaded_file( $_FILES['Filedata']['tmp_name'], $dest_file ) ) {
			self::check_safemode( WINABSPATH . $gallery->path );
	...
	static function check_safemode( $foldername ) {
		...
		if ( SAFE_MODE ) {
			...
			if ( $script_uid != $folder_uid ) {
				...
				$message .= '<br />' . sprintf( __( 'When safe_mode is on, PHP checks to see if the owner (%1$s) of the current script matches the owner (%2$s) of the file to be operated on by a file function or its directory', 'nggallery' ), $script_uid, $folder_uid );
				nggGallery::show_error( $message );
				return false;
			}
		}
	...
	static function move_images( $pic_ids, $dest_gid ) {
		...
		foreach ( $images as $image ) {
			...
			if ( ! @rename( $image->imagePath, $destination_path ) ) {
				$errors .= sprintf(
					__( 'Failed to move image %1$s to %2$s', 'nggallery' ),
					'<strong>' . esc_html( $image->filename ) . '</strong>',
					esc_html( $destination_path )
				) . '<br />';
				continue;
			}
	...
	static function copy_images( $pic_ids, $dest_gid ) {
		...
		foreach ( $images as $image ) {
			...
			if ( ! @copy( $image->imagePath, $destination_file_path ) ) {
				$errors .= sprintf(
					__( 'Failed to copy image %1$s to %2$s', 'nggallery' ),
					esc_html( $image->filename ),
					esc_html( $destination_file_path )
				) . '<br />';
				continue;
			}
			...
			if ( $tmp_prefix != '' ) {
				$messages .= sprintf(
					__( 'Image %1$s (%2$s) copied as image %3$s (%4$s) &raquo; The file already existed in the destination gallery.', 'nggallery' ),
					$image->pid,
					esc_html( $image->filename ),
					$new_pid,
					esc_html( $destination_file_name )
				) . '<br />';
			} else {
				$messages .= sprintf(
					__( 'Image %1$s (%2$s) copied as image %3$s (%4$s)', 'nggallery' ),
					$image->pid,
					esc_html( $image->filename ),
					$new_pid,
					esc_html( $destination_file_name )
