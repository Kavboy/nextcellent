...
class NGG_Installer {
	...
	PRIMARY KEY  (id)
		...
		if ( ! $wpdb->get_var( "SHOW TABLES LIKE '$nggpictures'" ) ) {
			update_option(
				'ngg_init_check',
				__(
					'NextCellent Gallery : Tables could not be created, please check your database settings',
					'nggallery'
				)
			);
	...
	public static function uninstall() {
		...
		self::ngg_remove_capability( 'NextGEN Gallery overview' );
		self::ngg_remove_capability( 'NextGEN Use TinyMCE' );
		self::ngg_remove_capability( 'NextGEN Upload images' );
		self::ngg_remove_capability( 'NextGEN Manage gallery' );
		self::ngg_remove_capability( 'NextGEN Edit album' );
		self::ngg_remove_capability( 'NextGEN Change style' );
		self::ngg_remove_capability( 'NextGEN Change options' );
	}
