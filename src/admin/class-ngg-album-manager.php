...
class NGG_Album_Manager implements NGG_Displayable {
	...
	private function update_album() {
		...
		$result = $wpdb->query(
			$wpdb->prepare(
				"UPDATE $wpdb->nggalbum SET slug= '%s', name= '%s', albumdesc= '%s', previewpic= %d, pageid= %d WHERE id = '%d'",
				$slug,
				$name,
				$desc,
				$prev,
		...
		<div class="wrap album" id="wrap">
				...
				<div class="albumnav tablenav">
					<div class="alignleft actions">
						...
						<?php if ( $this->currentID > 0 ) { ?>
							<input class="button-primary" type="submit" name="update" value="
							<?php
							esc_attr_e(
								'Update',
								'nggallery'
							...
							<?php if ( nggGallery::current_user_can( 'NextGEN Edit album settings' ) ) { ?>
								<input class="button-secondary" type="submit" name="showThickbox" value="
								<?php
								esc_attr_e(
									'Edit album',
									'nggallery'
							...
							<?php if ( nggGallery::current_user_can( 'NextGEN Add/Delete album' ) ) { ?>
								<input class="button-secondary action " type="submit" name="delete" value="
								<?php
								esc_attr_e(
									'Delete',
									'nggallery'
								...
								?>
									" onclick="javascript:check=confirm('
									<?php
									echo esc_js(
										'Delete album?',
										'nggallery'
						...
						<?php } else { ?>
							<?php if ( nggGallery::current_user_can( 'NextGEN Add/Delete album' ) ) { ?>
								...
								<input class="button-secondary action" type="submit" name="add" value="
								<?php
								esc_attr_e(
									'Add',
									'nggallery'
			...
			<div>
				<div style="float:right;">
					...
					<?php
					esc_attr_e(
						'Show / hide used galleries',
						'nggallery'
					);
					?>
						" id="toggle_used"><?php esc_html_e( '[Show all]', 'nggallery' ); ?></a>
					| <a href="#" title="
					<?php
					esc_attr_e(
						'Maximize the widget content',
						'nggallery'
					);
					?>
						" id="all_max"><?php esc_html_e( '[Maximize]', 'nggallery' ); ?></a>
					| <a href="#" title="
					<?php
					esc_attr_e(
						'Minimize the widget content',
						'nggallery'
					);
					?>
						" id="all_min"><?php esc_html_e( '[Minimize]', 'nggallery' ); ?></a>
				</div>
				<?php
				esc_html_e(
					'After you create and select a album, you can drag and drop a gallery or another album into your new album below',
					'nggallery'
				);
				?>
			</div>
			...
			<div class="container">
				...
				<div class="widget target-album widget-left">
					...
					<div class="widget-top">
						...
						?>
								<?php echo $album->id . ': ' . esc_html( $album->name ); ?> </h3>
					</div>
				...
				<div id="editalbum" style="display: none;">
					<form id="form-edit-album" method="POST" accept-charset="utf-8">
						...
						<table width="100%">
							...
							<tr>
								<th>
									...
									<select id="previewpic" name="previewpic" style="width:95%">
										...
										<?php
										if ( $album->previewpic == 0 ) {
											echo '<option value="0" selected="selected">' . __(
												'No picture',
												'nggallery'
											) . '</option>';
										} else {
							...
							<tr>
								<td class="submit">
									<input type="submit" class="button-primary" name="update_album" value="
									<?php
									esc_attr_e(
										'OK',
										'nggallery'
									...
									?>
										"/>
									&nbsp;
									<input class="button-secondary dialog-cancel" type="reset" value="
									<?php
									esc_attr_e(
										'Cancel',
										'nggallery'
		...
		echo '<div id="gid-' . $prefix . $obj['id'] . '" class="groupItem' . $used . '">
				<div class="innerhandle">
					<div class="item_top ' . $class . '">
						...
						ID: ' . $obj['id'] . ' | ' . wp_html_excerpt(
						esc_html( nggGallery::i18n( $obj['title'] ) ),
						25
					) . '
					</div>
					<div class="itemContent">
							...
							<p><strong>' . __(
								'Name',
								'nggallery'
							) . ': </strong>' . esc_html( nggGallery::i18n( $obj['name'] ) ) . '</p>
							<p><strong>' . __(
								'Title',
								'nggallery'
							) . ': </strong>' . esc_html( nggGallery::i18n( $obj['title'] ) ) . '</p>
							<p><strong>' . __(
								'Page',
								'nggallery'
							) . ': </strong>' . esc_html( nggGallery::i18n( $obj['pagenname'] ) ) . '</p>
