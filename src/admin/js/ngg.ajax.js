		...
		run( index ) {
			...
			const req = $.ajax( {
				...
				success( msg ) {
					switch ( parseInt( msg ) ) {
						case -1:
							nggProgressBar.addNote(
								nggAjax.settings.permission
							);
							break;
						case 0:
						...
						default:
							...
							nggProgressBar.addNote(
								'<strong>ID ' +
									nggAjax.settings.ids[ index ] +
									':</strong> ' +
									nggAjax.settings.failure,
								msg
							);
							break;
					}
				...
				complete() {
					...
					if ( index < nggAjax.settings.ids.length )
						nggAjax.run( index );
					else nggProgressBar.finished();
				},
			} );
		},

		readIDs( index, operation, next ) {
			s = this.settings;
		...
		init( s ) {
			...
			if ( this.settings.operation.substring( 0, 8 ) === 'gallery_' ) {
				...
				this.readIDs(
					index,
					'get_image_ids',
					this.settings.operation.substring( 8 )
				);
