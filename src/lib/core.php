...
class nggGallery {
	...
	static function get_thumbnail_url( $imageID, $picturepath = '', $fileName = '' ) {
		...
		// set gallery url
		$folder_url     = site_url() . '/' . $picturepath . self::get_thumbnail_folder( $picturepath, false );
			...
			if ( ! is_dir( $gallerypath . '/thumbs/' ) ) {
				if ( ! wp_mkdir_p( $gallerypath . '/thumbs/' ) ) {
					if ( SAFE_MODE ) {
						...
						self::show_error( __( 'Unable to create directory ', 'nggallery' ) . $gallerypath . '/thumbs !' );
					}
					return false;
				}
				return '/thumbs/';
	...
	static function get_thumbnail_folder( $gallerypath, $include_Abspath = true ) {
		return self::create_thumbnail_folder( $gallerypath, $include_Abspath );
	}
	...
	static function graphic_library() {
		...
		if ( $ngg_options['graphicLibrary'] == 'im' ) {
			return NGGALLERY_ABSPATH . 'src/lib/imagemagick.inc.php';
		} else {
			return NGGALLERY_ABSPATH . 'src/lib/gd.thumbnail.inc.php';
		}
		...
		if ( ( function_exists( 'memory_get_usage' ) ) && ( ini_get( 'memory_limit' ) ) ) {
			...
			foreach ( $sizes as $size ) {
				...
				if ( $freeMemory < round( $size['width'] * $size['height'] * 5.09 ) ) {
					$result = sprintf( __( 'Note : Based on your server memory limit you should not upload larger images then <strong>%1$d x %2$d</strong> pixel', 'nggallery' ), $size['width'], $size['height'] );
					return $result;
				}
			}
		}
