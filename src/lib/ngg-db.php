	...
	class nggdb {
		...
		static function delete_gallery( $id ) {
			...
			foreach ( $albums as $album ) {
				...
				self::update_album( $albumid, false, false, false, $new_galleries );
			}
		...
		static function update_album( $id, $name = false, $previewpic = false, $description = false, $sortorder = false, $pageid = false ) {
			...
			// slug must be unique, we use the title for that
			$slug = self::get_unique_slug( sanitize_title( $name ), 'album' );
		...
		static function find_images_in_list( $pids, $exclude = false, $order = 'NOTSET' ) {
			...
			$qry = self::find_images_query( $pids, $exclude, $order ); // Build query to get images
		...
		static function add_image( $id = false, $filename = false, $description = '', $alttext = '', $meta_data = false, $post_id = 0, $imagedate = '0000-00-00 00:00:00', $exclude = 0, $sortorder = 0 ) {
			...
			if ( false === $wpdb->query(
				$wpdb->prepare(
					"INSERT INTO $wpdb->nggpictures (image_slug, galleryid, filename, description, alttext, meta_data, post_id, imagedate, exclude, sortorder)
		...
		static function add_album( $name = false, $previewpic = 0, $description = '', $sortorder = 0, $pageid = 0 ) {
			...
			if ( false === $wpdb->query(
				$wpdb->prepare(
					"INSERT INTO $wpdb->nggalbum (name, slug, previewpic, albumdesc, sortorder, pageid)
													 VALUES (%s, %s, %d, %s, %s, %d)",
					$name,
					$slug,
					$previewpic,
			...
			if ( $images ) {
				foreach ( $images as $key => $image ) {
					...
					if ( ! array_key_exists( $image->galleryid, $gallery_cache ) ) {
						$gallery_cache[ $image->galleryid ] = self::find_gallery( $image->galleryid );
					}
